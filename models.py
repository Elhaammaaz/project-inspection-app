...
class ProjectInspection(db.Model):
    """Project Inspection model for storing project information"""
    ...
    gross_built_area = db.Column(db.Float, nullable=True)  # in m²
    ...
    system_threshold = db.Column(db.Float, nullable=True)  # in %
    
    # Assessment Results
    inspection_result = db.Column(db.String(100), nullable=True)  # e.g., "Passed but Need Attention"
    ...
    government_compliance = db.Column(db.String(50), nullable=True)  # e.g., "Complied", "Not Complied"
    ...
    # Systems Performance (stored as JSON string for flexibility)
    systems_data = db.Column(db.JSON, nullable=True)
